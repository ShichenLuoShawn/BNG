...
class ModelSTAGIN(nn.Module):
    ...
    def forward(self, v, a, t, sampling_endpoints):
        ...
        # print(time_encoding[[0,-1],0,:,:])
        h = torch.cat([a, time_encoding], dim=3)
        ...
        for layer, (G, R, T, L) in enumerate(zip(self.gnn_layers, self.readout_modules, self.transformer_modules, self.linear_layers)):
            ...
            reg_ortho += (matrix_inner/(matrix_inner.max(-1)[0].unsqueeze(-1)+1e-9) - torch.eye(num_nodes, device=matrix_inner.device)).triu().norm(dim=(1,2)).mean()
